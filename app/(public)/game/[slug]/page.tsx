...
export default function GamePlayPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        // Fetch game + leaderboard simultaneously — use cache with revalidation
        const [gameRes, lbRes] = await Promise.all([
          fetch(`/api/games/${params.slug}`, { next: { revalidate: 60 } }),
          fetch(`/api/leaderboard?gameSlug=${params.slug}`),
        ]);

        if (cancelled) return;

        const [gameData, lbData] = await Promise.all([gameRes.json(), lbRes.json()]);

        if (gameData.success) {
          setGame(gameData.data);

          // Fetch related games — non-blocking
          if (gameData.data.categories?.length) {
            const catId = typeof gameData.data.categories[0] === 'object'
              ? gameData.data.categories[0]._id
              : gameData.data.categories[0];
        ...
        if (lbData.success) setScores(lbData.data);
      } catch {
        setRelatedLoading(false);
      }
